...
for (rotation_folder in folder_list){
  ...
  rotation_angle <- gsub(".*r_(\\d+)$", "\\1", rotation_folder)
  folder_name <- gsub(".*/([^/]+)/?$", "\\1", rotation_folder)
  
  # Create the folder path
  figure_output_folder <- file.path("figures", "rotation", folder_name)
  ...
  for (i in 1:(length(raster_list_full)-1)) {
    ...
    p + ggtitle(paste0("Street angle: ", rotation_angle-30 ,"° ", "- Scenario: ", scenario)) -> p
