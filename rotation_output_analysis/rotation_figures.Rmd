...
scenario_summary_df <- scenario_summary_df %>%
  mutate(street_angle = factor(street_angle, ordered = TRUE),
         transveg = factor(transveg, levels = unique(transveg), labels = seq(5,50,5), ordered = TRUE),
         street_orientation = case_when(
           street_angle == 15 ~ "base",
           street_angle == 45 ~ "NE-SW",
           street_angle == 90 ~ "EW",
           street_angle == 135 ~ "NW-SE",
           street_angle == 180 ~ "NS",
           street_angle == 225 ~ "NW-SE",
           street_angle == 270 ~ "EW",
           street_angle == 315 ~ "NE-SW",
           street_angle == 360 ~ "NS",
           TRUE ~ NA_character_) %>%
           factor(levels = c("base", "NS", "EW", "NE-SW", "NW-SE")))
...
scenario_summary_df %>%
  ...
  ggplot(aes(x = transveg, y = mean_delta_Tmrt, colour = street_angle, linetype)) +
  geom_point() +
  geom_line(aes(linetype = street_orientation)) +
  theme_ipsum() -> p


for (i in 1:length(unique(scenario_summary_df$street_angle))) {
  scenario_df <- filter()
...
summary_slopes <- right_join(scenario_slopes, scenario_summary_df %>% mutate(street_angle = as.numeric(as.character(street_angle))) , by = "street_angle")
